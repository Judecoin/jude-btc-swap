...
// TODO(Franck): Use helper functions from jude-btc instead of re-writing them
// here
...
where
    W: Transfer,
{
    let S_a = jude::PublicKey::from_private_key(&jude::PrivateKey { scalar: state3.s_a });
    ...
    // TODO(Franck): Wait for jude to be confirmed once
    //  Waiting for jude confirmations should not be done in here, but in a separate
...
pub fn extract_jude_private_key(
    published_refund_tx: bitcoin::Transaction,
    tx_refund: &TxRefund,
    s_a: jude::Scalar,
    a: bitcoin::SecretKey,
    S_b_bitcoin: bitcoin::PublicKey,
) -> Result<jude::PrivateKey> {
    let s_a = jude::PrivateKey { scalar: s_a };
    ...
    let s_b = bitcoin::recover(S_b_bitcoin, tx_refund_sig, tx_refund_encsig)
        .context("Failed to recover jude secret key from Bitcoin signature")?;
    let s_b = jude::private_key_from_secp256k1_scalar(s_b.into());
