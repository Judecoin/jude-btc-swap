...
use harness::bob_run_until::is_jude_locked;
...
#[tokio::test]
async fn concurrent_bobs_after_jude_lock_proof_sent() {
    harness::setup_test(SlowCancelConfig, |mut ctx| async move {
        let (bob_swap_1, bob_join_handle_1) = ctx.bob_swap().await;

        let swap_id = bob_swap_1.id;

        let bob_swap_1 = tokio::spawn(bob::run_until(bob_swap_1, is_jude_locked));
        ...
        assert!(matches!(bob_state_1, BobState::JudeLocked { .. }));
        ...
        assert!(matches!(bob_state_2, BobState::JudeRedeemed { .. }));
        ...
        let (bob_swap_1, _) = ctx
            ...
            .await;
        assert!(matches!(bob_swap_1.state, BobState::JudeLocked { .. }));
        ...
        assert!(matches!(bob_state_1, BobState::JudeRedeemed { .. }));
