...
use jude::cryptonote::hash::Hash;
use jude::util::ringct;
use jude::PublicKey;
...
#[jsonrpc_client::implement(JudedRpc)]
#[derive(Debug, Clone)]
pub struct Client {
...
impl Client {
    /// New local host juded RPC client.
        ...
        Res: DeserializeOwned,
    {
        let response = self
            .inner
            .post(url)
            .body(jude_epee_bin_serde::to_bytes(&request)?)
            .send()
            .await?;
        ...
        Ok(jude_epee_bin_serde::from_bytes(body)?)
    }
}
...
// We should be able to use jude-rs for this but it does not include all
// the fields.
#[derive(Clone, Debug, Deserialize)]
...
pub struct GetBlockResponse {
    #[serde(with = "jude_serde_hex_block")]
    pub blob: jude::Block,
}
...
mod jude_serde_hex_block {
    use super::*;
    use jude::consensus::Decodable;
    use serde::de::Error;
    use serde::{Deserialize, Deserializer};
    use std::io::Cursor;

    pub fn deserialize<'de, D>(deserializer: D) -> Result<jude::Block, D::Error>
    where
        D: Deserializer<'de>,
    {
        ...
        let block = jude::Block::consensus_decode(&mut cursor).map_err(D::Error::custom)?;
