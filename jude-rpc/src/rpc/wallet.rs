...
/// JSON RPC client for jude-wallet-rpc.
#[derive(Debug, Clone)]
pub struct Client {
...
impl Client {
    ...
    /// Transfers `amount` judej from `account_index` to `address`.
    ...
    /// Transfers judej from `account_index` to `destinations`.
    ...
    /// Get wallet block height, this might be behind juded height.
