...
pub const judeD_RPC_PORT: u16 = 48081;
...
impl Image for jude {
    ...
    fn wait_until_ready<D: Docker>(&self, container: &Container<'_, D, Self>) {
        ...
        let additional_sleep_period =
            var("jude_ADDITIONAL_SLEEP_PERIOD").map(|value| value.parse());
...
impl Default for Args {
    fn default() -> Self {
        Self {
            image_args: ImageArgs::judedArgs(judedArgs::default()),
        }
    }
}
...
impl ImageArgs {
    fn args(&self) -> String {
        match self {
            ImageArgs::judedArgs(juded_args) => juded_args.args(),
...
impl judedArgs {
    // Return juded args as is single string so we can pass it to bash.
    fn args(&self) -> String {
        let mut args = vec!["juded".to_string()];
...
impl WalletArgs {
    ...
    // Return jude-wallet-rpc args as is single string so we can pass it to bash.
    fn args(&self) -> String {
        let mut args = vec!["jude-wallet-rpc".to_string()];
