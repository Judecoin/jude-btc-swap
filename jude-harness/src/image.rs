...
pub const JUDED_DAEMON_CONTAINER_NAME: &str = "juded";
pub const JUDED_DEFAULT_NETWORK: &str = "jude_network";

/// The port we use for all RPC communication.
///
/// This is the default when running juded.
/// For `jude-wallet-rpc` we always need to specify a port. To make things
...
impl Image for Juded {
    ...
    fn descriptor(&self) -> String {
        "rinocommunity/jude:v0.18.0.0".to_owned()
    }
...
pub struct JudeWalletRpc {
    args: JudeWalletRpcArgs,
}

impl Image for JudeWalletRpc {
    type Args = JudeWalletRpcArgs;
    type EnvVars = HashMap<String, String>;
    type Volumes = HashMap<String, String>;
    type EntryPoint = str;
...
impl JudeWalletRpc {
    pub fn new(name: &str, daemon_address: String) -> Self {
        Self {
            args: JudeWalletRpcArgs::new(name, daemon_address),
        }
    }
}

#[derive(Debug, Clone)]
pub struct JudedArgs {
    pub regtest: bool,
    pub offline: bool,
...
impl Default for JudedArgs {
    fn default() -> Self {
        Self {
            ...
            fixed_difficulty: 1,
            data_dir: "/jude".to_string(),
        }
    }
}

impl IntoIterator for JudedArgs {
    type Item = String;
...
impl Default for JudeWalletRpcArgs {
    fn default() -> Self {
        unimplemented!("A default instance for `JudeWalletRpc` doesn't make sense because we always need to connect to a node.")
    }
}

impl JudeWalletRpcArgs {
...
impl IntoIterator for JudeWalletRpcArgs {
    type Item = String;
    type IntoIter = ::std::vec::IntoIter<String>;

    fn into_iter(self) -> <Self as IntoIterator>::IntoIter {
        let mut args = vec![
            "jude-wallet-rpc".to_string(),
